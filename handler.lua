...
function plugin:access(config)
  ...
  local latency
  if config.request_percentage > random(100) then
    latency = config.minimum_latency_msec +
              random(config.minimum_latency_msec, config.maximum_latency_msec)
    ngx.sleep(latency/1000)
  end

  if config.add_header then
    ngx.header["X-Kong-Random-Latency"] = latency or "none"
  end
